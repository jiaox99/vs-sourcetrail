...
namespace CoatiSoftware.SourcetrailExtension
{
	public class OptionPageGrid : DialogPage
	{
		...
		[Description("Enables or disables file logging for the extension")]
	...
	public sealed class SourcetrailExtensionPackage : AsyncPackage
	{
		...
		protected async override System.Threading.Tasks.Task InitializeAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
		{
			...
			// register callbacks for changes of extension settings
			...
			if (null != mcs)
			{
				...
				CommandID createCdbID = new CommandID(GuidList.guidSourcetrailExtensionCmdSet, (int)PkgCmdIDList.cmdidSourcetrailCreateCdb);
				CommandID openLogDirID = new CommandID(GuidList.guidSourcetrailExtensionCmdSet, (int)PkgCmdIDList.cmdidSourcetrailOpenLogFolder);
		...
		{
			Logging.Logging.LogInfo("Solution closed, disabling extension UI");
